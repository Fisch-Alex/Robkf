...
std::list < struct Particle > Kalman_step(const std::list<Eigen::MatrixXd> & considered_Y, int horizon, std::list<std::list<struct Particle> > & Particle_history,  const Eigen::MatrixXd & A, 
...
const std::vector <double> & prob_inn, const std::vector <double> & prob_add, int Particle_Number,  std::list<Eigen::MatrixXd> Y_list, const std::vector<int> & Number_of_resamples) 
{
	...
	for (counter = 0; counter < horizon; counter++)
	{
		...
		for (Particle_iterator = History_iterator->begin(); Particle_iterator != History_iterator->end(); Particle_iterator++)
		{
			...
			Candidates_addition = Get_Particle_Innovative_Descendents(*Particle_iterator, *Y_iterator, *C_iterator, sigma_hat, Sigma_Inn, Num_Descendents, s, prob_inn,*Sampling_iterator, horizon);
	...
	for (Particle_iterator = Particle_history.back().begin(); Particle_iterator != Particle_history.back().end(); Particle_iterator++)
	{
		...
		Candidates_addition = Get_Particle_Descendents(*Particle_iterator, considered_Y.front(), sigma_tilde, Sigma_Add, Num_Descendents, s, prob_add);
